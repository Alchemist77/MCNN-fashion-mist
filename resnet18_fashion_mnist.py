...
NUM_CLASSES = 10
EARLY_STOPPING = 25
OUTPUT_DIR = 'outputs/output_Resnet18/'
...
def classifaction_report_csv(report):
    report_data = []
    lines = report.split('\n')
    with open('results/Resnet18_wo_aug.txt', 'w') as f:
        for line in lines:
            f.write(line)
            f.write('\n')
...
np.savetxt("loss/Resnet18_train.txt", train_loss_data, delimiter=',')
np.savetxt("loss/Resnet18_test.txt", val_loss_data, delimiter=',')
np.savetxt("accuracy/Resnet18_train.txt", train_accuracy_data, delimiter=',')
np.savetxt("accuracy/Resnet18_test.txt", val_accuracy_data, delimiter=',')

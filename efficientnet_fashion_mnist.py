...
NUM_CLASSES = 10
EARLY_STOPPING = 25
OUTPUT_DIR = 'outputs/output_Efficientnet/'
...
#original  data
all_transforms = transforms.Compose([transforms.Resize(32),transforms.ToTensor(),transforms.Normalize((0.1307,), (0.3081,)), ])
...
def classifaction_report_csv(report):
    report_data = []
    lines = report.split('\n')
    with open('results/Efficientnet_wo_aug.txt', 'w') as f:
        for line in lines:
            f.write(line)
            f.write('\n')

model = EfficientNet.from_name("efficientnet-b0", num_classes=10, in_channels=1)
...
np.savetxt("loss/Efficientnet_train.txt", train_loss_data, delimiter=',')
np.savetxt("loss/Efficientnet_test.txt", val_loss_data, delimiter=',')
np.savetxt("accuracy/Efficientnet_train.txt", train_accuracy_data, delimiter=',')
np.savetxt("accuracy/Efficientnet_test.txt", val_accuracy_data, delimiter=',')
